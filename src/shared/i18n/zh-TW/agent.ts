...
export const agent: Record<keyof typeof agentZh, string> = {
  ...
  'agent.subtitleAssistant': 'AI Agent 驅動 · 可操作桌面',
  ...
  'agent.errNoAgentDetail': '桌面助理由本機已安裝的 AI Agent 驅動（Claude Code / ZCode 等），請先安裝任一個。',
  ...
  'agent.welcomeDesc': '由本機 AI Agent 驅動，透過桌面工具直接操作這台電腦：啟動應用程式、找檔案、記待辦、調整音量，需要時還能開瀏覽器查資料。所有工具呼叫皆留稽核記錄。',
  'agent.computerUseHint': '桌面點擊/鍵入（computer use）預設關閉，可在設定頁「允許操作電腦」中開啟',

  // 桌面 MCP 分發卡片
  'agent.mcpCardTitle': '桌面 MCP · 分發到 AI Agent',
  ...
  'agent.noAgents': '未偵測到支援 MCP 分發的 AI Agent（安裝 Claude Code / Codex 等後會自動出現）。',
