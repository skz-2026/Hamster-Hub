...
export const agent = {
  ...
  'agent.subtitleAssistant': 'AI Agent 驱动 · 可操作桌面',
  ...
  'agent.errNoAgentDetail': '桌面助手由本机已安装的 AI Agent 驱动（Claude Code / ZCode 等），请先安装任意一个。',
  ...
  'agent.welcomeDesc': '由本机 AI Agent 驱动，经桌面工具直接操作这台电脑：启动应用、找文件、记待办、调音量，需要时还能开浏览器查资料。所有工具调用留审计记录。',
  'agent.computerUseHint': '桌面点击/键入（computer use）默认关闭，可在设置页「允许操作电脑」中开启',

  // 桌面 MCP 分发卡片
  'agent.mcpCardTitle': '桌面 MCP · 分发到 AI Agent',
  ...
  'agent.noAgents': '未检测到支持 MCP 分发的 AI Agent（安装 Claude Code / Codex 等后自动出现）。',
