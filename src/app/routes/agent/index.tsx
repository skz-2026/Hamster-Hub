...
/**
 * AI 助手对话页（Agent 接入点）：OpenAI 兼容会话补全。
 * 配置在设置页（AI 分区：base_url / model / api_key）；未配置时展示引导。
 * 会话保存在内存（路由内），后续 Agent 扩展：工具调用 / 流式输出 / 桌面操作。
 */
export default function AgentPage() {
  ...
  // Spotlight「问 AI」入口：读取暂存问题并自动发送（一次性）
  useEffect(() => {
    commands
  ...
  return (
    <div className="relative flex h-full flex-col overflow-hidden rounded-2xl bg-[radial-gradient(120%_120%_at_20%_0%,#232032_0%,#16141d_55%,#1a1520_100%)] text-white">
      ...
      {/* 顶栏 */}
      <header className="relative flex shrink-0 items-center gap-2.5 px-6 py-4">
        ...
        <div>
          ...
          <p className="text-[11px] text-white/45">会话补全 · Agent 能力将在此扩展</p>
        </div>
        {messages.length > 0 && (
          <button
            onClick={() => {
      ...
      {/* 消息列表 */}
      <div ref={listRef} className="relative min-h-0 flex-1 space-y-4 overflow-y-auto px-6 pb-4">
