...
- **源**：微博 / 知乎 / B 站等公开热榜接口，Rust `reqwest` 服务端抓取（无 CORS 问题），每源独立失败降级；具体端点在实现处维护，接口变动只影响对应源适配器。
...
- **桌面主页（桌面模式默认页 /desktop）**：全屏壁纸 + 中央问候语/超大时钟/农历 + 大搜索框 + 小组件卡片行 + 快捷入口药丸；无顶部菜单栏、无侧栏、无窗口控件（完全沉浸）。
