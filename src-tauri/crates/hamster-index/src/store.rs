...
use hamster_core::{IndexStatus, SearchHit, SearchQuery};
...
pub struct ApplyChunk<'a> {
    ...
    pub parse: &'a hamster_core::ChunkParse,
}
...
impl IndexStore {
    pub fn open(path: &Path) -> Result<Self> {
        if let Some(dir) = path.parent() {
            std::fs::create_dir_all(dir).map_err(|e| HamsterError::io(dir, e))?;
        }
        let conn = Connection::open(path)
            .map_err(|e| HamsterError::Other(format!("打开会话索引失败：{e}")))?;
    ...
    /// 会话列表（agent-cli 只读共享面）：按源文件 mtime 倒序（≈最近活跃优先），
    /// `agent` 过滤可选。
    pub fn list_sessions(
        &self,
        agent: Option<&str>,
        limit: usize,
    ) -> Result<Vec<hamster_core::SessionMeta>> {
        let conn = lock_ok(&self.conn);
        ...
        let map_row = |r: &rusqlite::Row| -> rusqlite::Result<hamster_core::SessionMeta> {
            Ok(hamster_core::SessionMeta {
                agent: r.get(0)?,
                session_key: r.get(1)?,
                title: r.get(2)?,
    ...
    /// 单个会话元数据（agent-cli session get 的头部；不存在返回 None）。
    pub fn session_meta(
        &self,
        agent: &str,
        session_key: &str,
    ) -> Result<Option<hamster_core::SessionMeta>> {
        let conn = lock_ok(&self.conn);
        conn.query_row(
            "SELECT agent, session_key, title, project_path, message_count, source_mtime, source_path
    ...
    pub fn messages_page(
        ...
        from_seq: i64,
        limit: i64,
    ) -> Result<hamster_core::SessionMessagesPage> {
        let conn = lock_ok(&self.conn);
        let total: i64 = conn
            .query_row(
        ...
        let messages = rows
            .into_iter()
            .map(
                |(seq, role, text, tool_name, at)| hamster_core::SnapshotMessage {
                    seq: seq.max(0) as usize,
            ...
            .collect();
        Ok(hamster_core::SessionMessagesPage { total, messages })
    }
    ...
    pub fn messages_around(
        ...
        seq: i64,
        window: i64,
    ) -> Result<Vec<hamster_core::SnapshotMessage>> {
        let conn = lock_ok(&self.conn);
        let mut stmt = conn
            .prepare(
                "SELECT seq, role, text, tool_name, at FROM messages
        ...
        for (seq, role, text, tool_name, at) in rows.flatten() {
            ...
            out.push(hamster_core::SnapshotMessage {
                seq: seq.max(0) as usize,
                role,
                text,
                tool_name,
...
fn parse_role(role: &str) -> hamster_core::SnapshotRole {
    match role {
        "assistant" => hamster_core::SnapshotRole::Assistant,
        "thinking" => hamster_core::SnapshotRole::Thinking,
        "tool" => hamster_core::SnapshotRole::Tool,
        "system" => hamster_core::SnapshotRole::System,
        _ => hamster_core::SnapshotRole::User,
    }
}

fn serde_role(role: hamster_core::SnapshotRole) -> &'static str {
    match role {
        hamster_core::SnapshotRole::User => "user",
        hamster_core::SnapshotRole::Assistant => "assistant",
        hamster_core::SnapshotRole::Thinking => "thinking",
        hamster_core::SnapshotRole::Tool => "tool",
        hamster_core::SnapshotRole::System => "system",
    }
}
...
fn sqlite_err(e: rusqlite::Error) -> HamsterError {
    HamsterError::Other(format!("会话索引错误：{e}"))
}

#[cfg(test)]
mod tests {
    use super::*;
    use hamster_core::model::session::{ChunkParse, SnapshotMessage, SnapshotRole};
    ...
    fn latest_for_project_matches_by_normalized_path() {
        let s = store();
        apply(&s, "codex", "old", Some("上游源仓库"), &["旧会话"], 1000);
        apply(&s, "codex", "new", Some("D:\\ksa\\上游"), &["新会话"], 2000);
        apply(&s, "codex", "other", Some("D:/other"), &["别处"], 3000);

        let found = s.latest_for_project("codex", "d:/ksa/上游/").unwrap();
        ...
        assert!(s
            ...
            .is_none());
        assert!(s
            .latest_for_project("claude", "上游源仓库")
            .unwrap()
            .is_none());
    }
