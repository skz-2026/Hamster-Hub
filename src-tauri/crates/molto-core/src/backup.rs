//! 备份与回滚：写入前快照到 ~/.molto/backups/<agent>/<timestamp>/，默认每 Agent 保留 20 份。
//!
...
impl BackupManager {
    ...
    pub fn snapshot(
        ...
        fs::create_dir_all(&dir).map_err(|e| MoltoError::io(&dir, e))?;
        ...
        fs::write(&content_path, content.as_bytes())
            .map_err(|e| MoltoError::io(&content_path, e))?;
        ...
        let raw = serde_json::to_string_pretty(&meta)
            .map_err(|e| MoltoError::Other(format!("序列化快照元数据失败：{}", e)))?;
        fs::write(&meta_path, format!("{}\n", raw)).map_err(|e| MoltoError::io(&meta_path, e))?;
    ...
    fn prune(&self, adapter: &str) -> Result<()> {
        ...
        let entries = match fs::read_dir(&dir) {
            ...
            Err(e) => return Err(MoltoError::io(&dir, e)),
        };
        ...
        while ids.len() > RETAIN_PER_AGENT {
            ...
            if let Err(e) = fs::remove_dir_all(&path) {
                if e.kind() != std::io::ErrorKind::NotFound {
                    return Err(MoltoError::io(&path, e));
                }
            }
        }
        Ok(())
    ...
    pub fn list(&self) -> Result<Vec<BackupMeta>> {
        ...
        let agents = match fs::read_dir(&self.root) {
            ...
            Err(e) => return Err(MoltoError::io(&self.root, e)),
        };
    ...
    pub fn read_snapshot(&self, backup_path: &str) -> Result<String> {
        read_text(Path::new(backup_path))?
            .ok_or_else(|| MoltoError::not_found(format!("快照文件 {}", backup_path)))
    }
