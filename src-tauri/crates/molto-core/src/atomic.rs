...
pub fn read_text(path: &Path) -> Result<Option<String>> {
    match fs::read_to_string(path) {
        ...
        Err(e) => Err(MoltoError::io(path, e)),
    }
}
...
pub fn write_text_atomic(path: &Path, content: &str) -> Result<()> {
    let parent = path
        .parent()
        .ok_or_else(|| MoltoError::config_invalid(format!("路径没有父目录：{}", path.display())))?;
    fs::create_dir_all(parent).map_err(|e| MoltoError::io(parent, e))?;

    let file_name = path
        .file_name()
        .map(|n| n.to_string_lossy().to_string())
        .unwrap_or_else(|| "molto-atomic".to_string());
    let tmp = tempfile::Builder::new()
        .prefix(&format!(".{}.molto-tmp-", file_name))
        .rand_bytes(6)
        .tempfile_in(parent)
        .map_err(|e| MoltoError::io(parent, e))?;

    {
        let mut handle = tmp
            .as_file()
            .try_clone()
            .map_err(|e| MoltoError::io(path, e))?;
        handle
            .write_all(content.as_bytes())
            .map_err(|e| MoltoError::io(path, e))?;
        handle.sync_all().map_err(|e| MoltoError::io(path, e))?;
    }
    // persist = rename 到目标路径；Windows 下等价于覆盖式移动
    tmp.persist(path)
        .map_err(|e| MoltoError::io(path, e.error))?;
    Ok(())
}
