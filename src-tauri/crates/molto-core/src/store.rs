//! ~/.molto 存储：config.json（单一事实源）、state.json（分发 hash 基准）、
...
impl Store {
    ...
    /// 默认根：MOLTO_HOME 环境变量覆盖 home 目录（沙箱/测试用），否则 ~/.molto。
    pub fn default_root() -> PathBuf {
        Self::default_home().join(".molto")
    }

    /// 用户 home（MOLTO_HOME 覆盖）：adapters 以 <home>/.claude 等探测，
    /// CLI 用它组装 registry（与 default_root 同一来源，避免两套逻辑）。
    pub fn default_home() -> PathBuf {
        match std::env::var("MOLTO_HOME") {
    ...
    pub fn load_config(&self) -> Result<StoreConfig> {
        match read_text(&self.config_path())? {
            ...
            Some(raw) => serde_json::from_str(&raw)
                .map_err(|e| MoltoError::json_parse(self.config_path(), e.to_string())),
        }
    }

    pub fn save_config(&self, config: &StoreConfig) -> Result<()> {
        let raw = serde_json::to_string_pretty(config)
            .map_err(|e| MoltoError::Other(format!("序列化源配置失败：{}", e)))?;
    ...
    pub fn load_state(&self) -> Result<SyncState> {
        match read_text(&self.root.join(STATE_FILE))? {
            ...
            Some(raw) => serde_json::from_str(&raw)
                .map_err(|e| MoltoError::json_parse(self.root.join(STATE_FILE), e.to_string())),
        }
    }
    ...
    pub fn record_applied_with_key(&self, key: &str, path: &Path, hash: String) -> Result<()> {
        ...
        let raw = serde_json::to_string_pretty(&state)
            .map_err(|e| MoltoError::Other(format!("序列化状态失败：{}", e)))?;
        write_text_atomic(&self.root.join(STATE_FILE), &format!("{}\n", raw))
    }

    pub fn append_history(&self, entry: &HistoryEntry) -> Result<()> {
        fs::create_dir_all(&self.root).map_err(|e| MoltoError::io(&self.root, e))?;
        let line = serde_json::to_string(entry)
            .map_err(|e| MoltoError::Other(format!("序列化历史失败：{}", e)))?;
        let mut f = fs::OpenOptions::new()
            .create(true)
            .append(true)
            .open(self.root.join(HISTORY_FILE))
            .map_err(|e| MoltoError::io(self.root.join(HISTORY_FILE), e))?;
        writeln!(f, "{}", line).map_err(|e| MoltoError::io(self.root.join(HISTORY_FILE), e))?;
        Ok(())
    }
    ...
    pub fn load_projects(&self) -> Result<Vec<String>> {
        match read_text(&self.root.join(PROJECTS_FILE))? {
            None => Ok(vec![]),
            Some(raw) => serde_json::from_str(&raw)
                .map_err(|e| MoltoError::json_parse(self.root.join(PROJECTS_FILE), e.to_string())),
        }
    }
    ...
    pub fn add_project(&self, dir: &str) -> Result<Vec<String>> {
        ...
        if !projects.iter().any(|p| p == dir) {
            ...
            let raw = serde_json::to_string_pretty(&projects)
                .map_err(|e| MoltoError::Other(format!("序列化项目失败：{}", e)))?;
