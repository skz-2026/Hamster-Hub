...
//!
//! 执行：cargo test -p molto-index --test real_home_smoke -- --ignored --nocapture

use std::path::PathBuf;

use molto_core::{Registry, SearchQuery};
use molto_index::{ingest_all, IndexStore};

#[test]
#[ignore = "读本机真实 Agent 会话（只读），仅手动冒烟"]
fn real_home_index_smoke() {
    let home = std::env::var("MOLTO_INDEX_SMOKE_HOME")
        .map(PathBuf::from)
        .unwrap_or_else(|_| {
...
fn build_registry(home: &std::path::Path) -> Registry {
    molto_adapters::build_registry(home)
}
