...
fn git(dir: &Path, args: &[&str]) -> Result<String> {
    let out = Command::new("git")
        ...
        .args(args)
        .output()
        .map_err(|e| HamsterError::Other(format!("无法启动 git（请确认已安装并在 PATH）：{e}")))?;
...
pub fn diff(dir: &Path, path: &str, staged: bool) -> Result<String> {
    if !staged && is_untracked(dir, path)? {
        let out = Command::new("git")
            ...
            .output()
            .map_err(|e| HamsterError::Other(format!("无法启动 git：{e}")))?;
        if out.status.code() == Some(1) || out.status.success() {
            return Ok(String::from_utf8_lossy(&out.stdout).into_owned());
        }
        let stderr = String::from_utf8_lossy(&out.stderr).trim().to_string();
        return Err(HamsterError::Other(format!("git diff 失败：{stderr}")));
    }
...
pub fn commit(dir: &Path, message: &str) -> Result<String> {
    ...
    if msg.is_empty() {
        return Err(HamsterError::config_invalid("提交信息不能为空"));
    }
