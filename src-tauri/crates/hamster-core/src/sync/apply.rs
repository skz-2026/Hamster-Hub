...
fn write_with_backup(
    ...
    let new_content = plan
        .new_content
        .as_deref()
        .ok_or_else(|| HamsterError::Other(format!("计划缺少新内容：{}", plan.path)))?;
...
fn adopt_external(
    ...
    let current = plan
        .current_content
        .as_deref()
        .ok_or_else(|| HamsterError::Other(format!("漂移文件不存在：{}", plan.path)))?;
...
mod tests {
    ...
    impl AgentAdapter for JsonAdapter {
        ...
        fn read_mcp(&self, raw: &str) -> Result<Vec<McpServer>> {
            let v: serde_json::Value = serde_json::from_str(raw)
                .map_err(|e| HamsterError::json_parse(self.root.join("cfg.json"), e.to_string()))?;
            let mut out = vec![];
        ...
        fn render_mcp(&self, current: Option<&str>, servers: &[McpServer]) -> Result<String> {
            let mut root: serde_json::Value = match current {
                Some(c) if !c.trim().is_empty() => serde_json::from_str(c).map_err(|e| {
                    HamsterError::json_parse(self.root.join("cfg.json"), e.to_string())
                })?,
    ...
    fn fixture() -> (tempfile::TempDir, Store, BackupManager, Registry) {
        ...
        let store = Store::new(dir.path().join("agent"));
        let backup = BackupManager::new(dir.path().join("agent").join("backups"));
    ...
    fn drift_requires_decision_and_skip_writes_nothing() {
        ...
        assert!(matches!(err, HamsterError::Drift { .. }));
