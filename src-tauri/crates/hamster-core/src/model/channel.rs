...
pub struct ChannelConfig {
    ...
    pub repo_dir: String,
    /// worktree 隔离根目录（默认 上游 store_root/worktrees）
...
/// 开发者经 issue 评论下达的指令（/上游 前缀协议）。
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize, TS)]
#[serde(rename_all = "camelCase")]
#[ts(export)]
pub enum ChannelCommand {
    /// /上游 approve —— 认可当前产出（M1 PR 已建时仅回执）
    Approve,
    /// /上游 redo: <反馈> —— 带反馈重跑开发
    Redo(String),
    /// 非 上游 指令
...
/// 解析 issue 评论为 上游 指令（纯函数，首行 /上游 前缀生效）。
pub fn parse_command(body: &str) -> ChannelCommand {
    let first = body.trim().lines().next().unwrap_or("").trim();
    if let Some(rest) = first.strip_prefix("/上游 approve") {
        let _ = rest;
        return ChannelCommand::Approve;
    }
    if let Some(rest) = first.strip_prefix("/上游 redo") {
...
mod tests {
    ...
    fn command_parsing_prefix_and_feedback() {
        assert_eq!(parse_command("/上游 approve"), ChannelCommand::Approve);
        assert_eq!(
            parse_command("/上游 approve 后续说明忽略"),
            ChannelCommand::Approve
        );
        assert_eq!(
            parse_command("/上游 redo: 登录按钮点击无响应，请修复"),
            ChannelCommand::Redo("登录按钮点击无响应，请修复".into())
        );
        assert_eq!(
            parse_command("/上游 redo 带中文冒号：样式错位"),
            // 反馈内容本身可含中文冒号：只有紧跟 redo 的分隔冒号会被剥除
            ChannelCommand::Redo("带中文冒号：样式错位".into())
        );
        assert_eq!(
            parse_command("普通评论 /上游 approve"),
            ChannelCommand::None
        );
        assert_eq!(parse_command("/上游saurus 绝不匹配"), ChannelCommand::None);
    ...
    fn task_id_and_branch_format() {
        ...
        assert_eq!(ChannelTask::branch_name(7), "上游/issue-7");
    }
}
