...
use hamster_core::model::mcp::{McpServer, Scope, Transport};
...
use hamster_core::model::streaming::{ProtocolDialect, StructuredChannel};
...
impl AgentAdapter for CodexAdapter {
    ...
    fn read_mcp(&self, raw: &str) -> Result<Vec<McpServer>> {
        ...
        let doc = raw
            .parse::<DocumentMut>()
            .map_err(|e| HamsterError::toml_parse(path, e.to_string()))?;
    ...
    fn render_mcp(&self, current: Option<&str>, servers: &[McpServer]) -> Result<String> {
        ...
        let parent = doc[SERVERS_KEY].as_table_mut().ok_or_else(|| {
            HamsterError::config_invalid(format!(
                "「{}」中 {} 不是表，无法写入 MCP 配置",
                path.display(),
                SERVERS_KEY
            ))
    ...
    /// 跨 Agent 会话共享 skill（v2.4）：~/.codex/skills/hamster-session-resolver/SKILL.md
...
",
    ...
    fn injected_context_skipped_for_title_and_role() {
        ...
        assert_eq!(
            ...
            hamster_core::SnapshotRole::System,
            "注入上下文降级 system"
        );
        assert_eq!(parse.messages[1].1.role, hamster_core::SnapshotRole::User);
...
"#;
    ...
    fn preserves_comments_and_unknown_keys() {
        ...
        assert!(out.contains("# MCP servers（仓鼠Hub 管理区起点）"));
