...
use hamster_core::model::mcp::{McpServer, Transport};
...
pub fn read_servers(raw: &str, key_path: &[&str], path: &Path) -> Result<Vec<McpServer>> {
    ...
    let root: Value =
        serde_json::from_str(raw).map_err(|e| HamsterError::json_parse(path, e.to_string()))?;
...
pub fn render_servers(
    ...
    *map = new_map;
    ...
    let mut out = serde_json::to_string_pretty(&root)
        .map_err(|e| HamsterError::Other(format!("序列化 JSON 失败：{}", e)))?;
    out.push('\n');
    Ok(out)
}
