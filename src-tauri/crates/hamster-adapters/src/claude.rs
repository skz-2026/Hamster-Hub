...
//! 上游 只允许改动顶层 `mcpServers` 键，其余字段一律原样保留；写入由引擎原子化执行。
...
use hamster_core::model::session::{
    ...
    fn render_deny_rules(
        ...
        let mut root: serde_json::Value = match current {
            Some(raw) if !raw.trim().is_empty() => serde_json::from_str(raw)
                .map_err(|e| hamster_core::HamsterError::json_parse(&path, e.to_string()))?,
            _ => serde_json::json!({}),
        };
        ...
        let permissions = root
            .as_object_mut()
            .ok_or_else(|| {
                hamster_core::HamsterError::config_invalid(format!(
                    "「{}」顶层不是对象，无法写入工具权限",
                    path.display()
                ))
            })?
        ...
        let mut out = serde_json::to_string_pretty(&root)
            .map_err(|e| hamster_core::HamsterError::Other(format!("序列化 JSON 失败：{e}")))?;
        out.push('\n');
        Ok(Some(out))
    }
}
...
impl AgentAdapter for ClaudeAdapter {
    ...
    /// 跨 Agent 会话共享 skill（v2.4）：~/.claude/skills/hamster-session-resolver/SKILL.md
    ...
    fn runtime(&self) -> Option<RuntimeSpec> {
        Some(RuntimeSpec {
            ...
            structured: Some(StructuredChannel {
                dialect: hamster_core::ProtocolDialect::ClaudeStream,
            ...
            task_mode: Some(hamster_core::TaskModeSpec {
...
",
        ...
        assert_eq!(parse.messages[0].1.role, hamster_core::SnapshotRole::User);
        assert!(parse.messages[0].1.at.is_some(), "时间戳应被解析");
        assert_eq!(
            parse.messages[1].1.role,
            hamster_core::SnapshotRole::Thinking
        );
        assert!(parse.messages[1].1.text.contains("token 逻辑"));
        assert_eq!(
            parse.messages[2].1.role,
            hamster_core::SnapshotRole::Assistant
        );
        assert_eq!(parse.messages[3].1.role, hamster_core::SnapshotRole::Tool);
  ...
  "mcpServers": {
    ...
    fn roundtrip_read_render_read() {
        ...
        match (&reparsed[0].transport, &original[0].transport) {
            (
                hamster_core::model::mcp::Transport::Stdio {
                    command: c1,
                    args: a1,
                    env: e1,
                },
    ...
    fn render_from_empty_creates_minimal() {
        let srv = vec![McpServer {
            name: "fs".into(),
            transport: hamster_core::model::mcp::Transport::Stdio {
                command: "npx".into(),
                args: vec!["-y".into()],
                env: Default::default(),
            },
