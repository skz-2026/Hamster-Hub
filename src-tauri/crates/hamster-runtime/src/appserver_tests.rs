...
use hamster_core::ProtocolDialect::CodexAppServer;
use hamster_core::StreamEventKind::*;
use serde_json::json;
...
fn item_completed_variants_map_correctly() {
    ...
    let cmd = map(
        "item/completed",
        json!({ "item": {
            "id": "i4", "type": "commandExecution", "command": "cargo test -p 上游",
...
mod acp_tests {
    use hamster_core::ProtocolDialect::Acp;
    use hamster_core::StreamEventKind::*;
    use serde_json::json;
...
fn acp_tool_call_diff_content_is_extracted() {
    ...
    let mapped = crate::appserver::normalize_event(
        "s1",
        hamster_core::ProtocolDialect::Acp,
        "session/update",
        &json!({ "update": upd }),
    );
    assert_eq!(
        mapped.events[0].kind,
        hamster_core::StreamEventKind::ToolItem
    );
