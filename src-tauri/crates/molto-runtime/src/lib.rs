...
use molto_core::error::MoltoError;

/// portable-pty 的错误统一映射为 MoltoError::Launch（保持全库单一错误类型）。
pub(crate) fn pty_err(program: &str, source: impl std::fmt::Display) -> MoltoError {
    MoltoError::Launch {
