...
//! `<home>/.molto/runtime/<key>.scrollback`——重开/续聊同一逻辑会话时回放，
//! 恢复「看到的上下文」。存储键 = `resume_key`（agent 侧稳定 sessionKey，
//! 跨重启可对齐）缺省回落 pty session_id；非法字符的键确定性 hex 编码。
//! 这是 Molto 自有 UI 数据的落盘，不触碰 Agent 会话文件（红线①/⑥：原子写、
//! 本地优先）。
...
impl ScrollbackStore {
    ...
    pub fn save(&self, session_id: &str, resume_key: Option<&str>, data: &str) -> Result<()> {
        ...
        std::fs::create_dir_all(&self.root).map_err(|e| MoltoError::io(self.root.clone(), e))?;
        let tmp = self.root.join(format!(".{key}.tmp"));
        std::fs::write(&tmp, data).map_err(|e| MoltoError::io(tmp.clone(), e))?;
        std::fs::rename(&tmp, &path).map_err(|e| MoltoError::io(path.clone(), e))?;
        self.evict_lru(&path);
        Ok(())
    }
    ...
    pub fn load(&self, session_id: &str, resume_key: Option<&str>) -> Result<Option<String>> {
        ...
        match std::fs::metadata(&path) {
            ...
            Err(e) => return Err(MoltoError::io(path, e)),
            Ok(meta) => {
