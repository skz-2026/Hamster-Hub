...
impl StreamSession {
    ...
    pub fn start(opts: StreamOptions) -> Result<Arc<Self>> {
        ...
        let program = if opts.channel.dialect == molto_core::ProtocolDialect::ClaudeStream {
            resolve_native(&opts.program).unwrap_or_else(|| opts.program.clone())
        } else {
            opts.program.clone()
        };
        let mut cmd = Command::new(&program);
        if std::env::var("MOLTO_STREAM_DEBUG").ok().as_deref() == Some("1") {
        ...
        if opts.channel.dialect == molto_core::ProtocolDialect::ClaudeStream {
            cmd.args(claude_resume_flags(opts.resume_key.as_deref(), opts.fork));
        }
        let mut child = cmd.spawn().map_err(|e| MoltoError::Launch {
            program: program.clone(),
            message: format!("启动结构化通道失败：{e}"),
        })?;
        let stdout = child.stdout.take().ok_or_else(|| MoltoError::Launch {
            program: opts.program.clone(),
            message: "通道进程无 stdout".into(),
        })?;
        let stdin = child.stdin.take().ok_or_else(|| MoltoError::Launch {
            program: opts.program.clone(),
        ...
        if let Some(err_out) = stderr {
            let debug_raw = std::env::var("MOLTO_STREAM_DEBUG").ok().as_deref() == Some("1");
        ...
        if session.dialect == molto_core::ProtocolDialect::ClaudeStream {
        ...
        if session.dialect == molto_core::ProtocolDialect::Acp {
            ...
            if opts.fork {
                return Err(MoltoError::Unsupported(
                ...
                .filter(|k| !k.is_empty())
            {
                Some(key) => {
                    ...
                    let resp = session.request(
                        "session/load",
                        serde_json::json!({ "sessionId": key, "cwd": cwd, "mcpServers": [] }),
                    )?;
                    let tid = resp
                        .get("sessionId")
                        .and_then(|i| i.as_str())
        ...
        let init = session.request(
            "initialize",
            serde_json::json!({ "clientInfo": { "name": "molto", "version": "0.1.0" } }),
        )?;
    ...
    pub fn send_turn(&self, text: &str, model: Option<&str>, effort: Option<&str>) -> Result<()> {
        if !self.is_running() {
            return Err(MoltoError::Other("会话已结束，无法发送".into()));
        }
        // claude：发轮次只需 stdin 用户行；thread_id 校验仅 codex 需要
        let thread_id = self.thread_id();
        if self.dialect != molto_core::ProtocolDialect::ClaudeStream && thread_id.is_empty() {
            return Err(MoltoError::Other("通道尚未就绪（无 thread）".into()));
        }
        if self.dialect == molto_core::ProtocolDialect::ClaudeStream {
        ...
        if self.dialect == molto_core::ProtocolDialect::Acp {
    ...
    fn apply_model_override(&self, model: &str) -> Result<()> {
        ...
        match config_id {
            ...
            None => Err(MoltoError::Other(format!(
        ...
        match rx.recv_timeout(std::time::Duration::from_secs(30)) {
            Ok(raw) => {
                let v: serde_json::Value = serde_json::from_str(&raw)
                    .map_err(|e| MoltoError::Other(format!("协议响应解析失败：{e}")))?;
            ...
            Err(_) => {
                ...
                Err(MoltoError::Other(format!("{method} 响应超时（30s）")))
            }
        }
    }
    ...
    fn answer_acp_server_request(&self, id: i64, method: &str, params: &serde_json::Value) {
        let response = match method {
            ...
            other => serde_json::json!({
                "jsonrpc": "2.0", "id": id,
                "error": { "code": -32601, "message": format!("molto 未支持该 agent 侧请求：{other}") }
            }),
        };
...
fn spawn_reader_thread(
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || {
            // 协议调试：MOLTO_STREAM_DEBUG=1 时原样输出每个原始行到 stderr
            let debug_raw = std::env::var("MOLTO_STREAM_DEBUG").ok().as_deref() == Some("1");
            eprintln!(
        ...
        .map(|_| ())
        .map_err(|e| MoltoError::Other(format!("流式读取线程启动失败：{e}")))
}
...
pub fn normalize_event(
    ...
    match method {
        ...
        "session/update" if dialect == molto_core::ProtocolDialect::Acp => {
            let upd = params
                .get("update")
                .cloned()
                .unwrap_or(serde_json::Value::Null);
