...
//!   这些 Agent 的配置格式未经本机实测，Molto 不接管、不猜测
//! - 程序名与参数以官方 registry
...
impl AgentAdapter for AcpAgentAdapter {
    ...
    fn read_mcp(&self, _raw: &str) -> Result<Vec<McpServer>> {
        Err(MoltoError::Unsupported(format!(
    ...
    fn runtime(&self) -> Option<RuntimeSpec> {
        Some(RuntimeSpec {
            ...
            structured: Some(StructuredChannel {
                dialect: molto_core::ProtocolDialect::Acp,
...
mod tests {
    ...
    fn find_program_resolves_absolute_path() {
        ...
        assert!(find_program("molto-no-such-agent-xyz").is_none());
    }
    ...
    fn runtime_declares_acp_channel_and_honest_caps() {
        for s in SPECS {
            ...
            assert_eq!(channel.dialect, molto_core::ProtocolDialect::Acp);
            assert_eq!(
                channel.args,
    ...
    fn config_surface_is_explicitly_unsupported() {
        let a = AcpAgentAdapter::new(Path::new("/h"), spec("opencode"));
        assert!(matches!(a.read_mcp("{}"), Err(MoltoError::Unsupported(_))));
