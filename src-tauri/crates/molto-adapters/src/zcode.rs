...
use molto_core::model::runtime::{PromptInject, RuntimeSpec};
use molto_core::model::session::SessionSummary;
...
mod tests {
    ...
    use super::*;
    use molto_core::model::mcp::Transport;
  ...
  "mcp": {
    ...
    fn workspaces_from_recent_projects() {
        ...
        std::fs::write(
            &f,
            r#"{"recentProjects":["D:\\ksa\\Molto","D:\\ksa\\game"],"other":1}"#,
        )
        .unwrap();
        let ws = read_zcode_workspaces(&f);
        assert_eq!(ws.len(), 2);
        assert_eq!(ws[0].dir, "D:\\ksa\\Molto");
    }
