...
pub fn build_registry(home: &Path) -> Registry {
    let mut adapters: Vec<Box<dyn molto_core::adapter::AgentAdapter>> = vec![
    ...
    adapters.extend(SPECS.iter().map(|s| {
        Box::new(AcpAgentAdapter::new(home, s)) as Box<dyn molto_core::adapter::AgentAdapter>
    }));
...
/// 启动程序解析：Molto 安装渠道（npm 全局）的副本优先，回退裸名交 PATH。
...
mod tests {
    ...
    fn unknown_tool_falls_back_to_path() {
        ...
        assert_eq!(resolve_program("molto-no-such-tool"), "molto-no-such-tool");
    }
}
