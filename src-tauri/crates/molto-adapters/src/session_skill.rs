...
//!
//! 各 Agent 的 skills 目录下由 SyncEngine 分发同名 `molto-session-resolver` skill，
//! 内容（本模块）对全部 Agent 一致：指引 agent 用只读 `molto session` 命令解析
//! `molto://session/<agent>/<sessionKey>` 引用。渲染是纯函数；文件为 Molto 全权
...
/// 由引擎在分发时引用 core 常量）。
pub const SESSION_SKILL_NAME: &str = "molto-session-resolver";

pub const SESSION_SKILL_BODY: &str = r#"---
name: molto-session-resolver
description: Resolve molto://session/<agent>/<sessionKey> references — read another AI agent's conversation from the local Molto index. Use whenever a message contains a molto://session link and the referenced conversation matters as context.
---

# Molto Session Resolver

`molto://session/<agent>/<sessionKey>` is a Molto workspace reference to a conversation of another AI coding agent (for example Claude Code or Codex). It is a pointer only: resolve it on demand with the command below. Never guess the referenced content from the link label.
...
1. Run the read-only command:
   ...
   ```
   molto session get --agent <agent> --id <sessionKey> --json
   ```

   Optional: `--last <n>` caps the returned messages (default 20, max 200). `molto session list [--agent <id>] --json` discovers which sessions exist.
...
- Reference-only: a `molto://session/...` link does not authorize resuming, deleting, or writing to that session, its project, or any agent configuration.
...
mod tests {
    ...
    fn render_is_stable_and_complete() {
        ...
        assert!(a.contains("molto://session/<agent>/<sessionKey>"));
        assert!(a.contains("molto session get --agent <agent> --id <sessionKey> --json"));
